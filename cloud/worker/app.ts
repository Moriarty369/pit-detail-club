...
async function input<T>(c: Context<Env>, schema: z.ZodType<T>): Promise<T> {
  let value;
  try {
...
function client(c: Context<Env>) {
  return createServerClient(c.env.SUPABASE_URL, c.env.SUPABASE_ANON_KEY, {
    cookieOptions: {
      name: c.env.PORTAL === "admin" ? "pit-admin" : "pit-client",
    },
    cookies: {
      getAll: () =>
...
async function identity(sb: SupabaseClient, user: User): Promise<SessionInfo> {
  const roleResult = await sb.rpc("pit_session_role");
  check(roleResult);
...
export function createApp() {
  ...
  app.post("/api/auth/register", async (c) => {
    if (c.env.PORTAL !== "customer" || c.env.EMAIL_ENABLED !== "true")
    ...
    if (result.data.session) {
      ...
      fail(503, "El registro requiere configurar la verificación del correo.");
    }
    return c.json(
      {
        message:
  ...
  app.post("/api/auth/logout", async (c) => {
    ...
    if (
      ...
      check(result);
    deleteCookie(c, "pit-recovery", { path: "/api" });
  ...
  app.post("/api/mfa/verify", async (c) => {
    ...
    check(result);
    c.set("identity", await identity(c.get("sb"), c.get("user")));
    return sessionResponse(c);
  });
  app.use("/api/*", async (c, next) => {
  ...
  app.post("/api/auth/password", async (c) => {
    const data = await input(
      c,
      z
        .object({ password, currentPassword: z.string().max(128).optional() })
        .strict(),
    );
    const sb = c.get("sb");
  ...
  app.use("/api/admin/*", async (c, next) => {
    if (c.env.PORTAL !== "admin" || c.get("identity").role !== "admin")
  ...
  app.get("/api/admin/customers/:id", async (c) => {
    ...
    const r = await c.get("sb").rpc("pit_admin_member", { p_customer: id });
    check(r);
    return c.json(r.data);
  });
  ...
  app.post("/api/admin/services/:id/void", async (c) => {
    const id = z.string().uuid().parse(c.req.param("id"));
    const d = await input(
      c,
    ...
    const r = await c
      .get("sb")
      .rpc("pit_void_service", { p_id: id, p_reason: d.reason });
    check(r);
    return c.json(r.data);
  });
  app.post("/api/admin/redemptions", async (c) => {
    const d = await input(
      c,
      z
        .object({
          customerId: z.string().uuid(),
          code: z.string().regex(/^[a-fA-F0-9]{10}$/),
          conditionsConfirmed: z.literal(true),
        })
        .strict(),
    );
  ...
  app.get("/api/admin/rules", async (c) => {
    const r = await c
      .get("sb")
      .from("club_rules")
      .select("threshold_cents,rappel_percent")
      .single();
    check(r);
    return c.json({
      thresholdCents: r.data!.threshold_cents,
      rappelPercent: r.data!.rappel_percent,
    });
  });
  ...
  app.get("/api/admin/audit", async (c) => {
    const r = await c
      .get("sb")
      .from("audit_events")
      .select("id,actor_id,customer_id,action,details,created_at")
