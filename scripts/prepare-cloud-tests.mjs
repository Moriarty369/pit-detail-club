...
const sb = createClient(values.API_URL, values.SERVICE_ROLE_KEY, {
...
for (const [email, name, role] of [
  ...
  if (result.error)
    throw new Error(
      "No se pudo crear la cuenta local de pruebas: " + result.error.message,
    );
  if (role === "admin") {
    const id = result.data.user.id;
    if (!/^[a-f0-9-]{36}$/.test(id))
      throw new Error("Identificador no válido.");
    execFileSync("psql", [values.DB_URL, "-v", "ON_ERROR_STOP=1"], {
      input: `update private.roles set role='admin' where user_id='${id}';`,
